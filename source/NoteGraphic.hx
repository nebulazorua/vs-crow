...
class NoteGraphic extends FlxSprite
{
	...
	public function new(?modifier='base',?skin='default',?noteType='')
	{
		super();
		this.behaviour = Json.parse(Paths.noteSkinText("behaviorData.json",'skins',skin,modifier,noteType));
		...
		setBaseGraphic(modifier,skin,noteType);

		animation.play("greenScroll");
	}

	public function setBaseGraphic(?modifier='base',?skin='default',?noteType=''){
		switch (behaviour.actsLike)
		{
			case 'pixel':
				//loadGraphic(Paths.image('weeb/pixelUI/arrows-pixels',"shared"), true, 17, 17);
				loadGraphic(Paths.noteSkinImage(behaviour.arguments.note.sheet, 'skins', skin, modifier, noteType),true,behaviour.arguments.note.gridSizeX,behaviour.arguments.note.gridSizeY);
			...
			default:
				frames = Paths.noteSkinAtlas(behaviour.arguments.spritesheet, 'skins', skin, modifier, noteType);
	...
	public function setDir(dir:Int=0,?sussy:Bool=false,?end:Bool=false){
		...
		if(sussy && behaviour.actsLike=='pixel'){
			if(end && !animation.curAnim.name.endsWith("end")){
				...
				loadGraphic(Paths.noteSkinImage(args.sheet, 'skins', skin, modifier, type),true,args.gridSizeX,args.gridSizeY);
			...
			loadGraphic(Paths.noteSkinImage(behaviour.arguments.note.sheet, 'skins', skin, modifier, type),true,behaviour.arguments.note.gridSizeX,behaviour.arguments.note.gridSizeY);
